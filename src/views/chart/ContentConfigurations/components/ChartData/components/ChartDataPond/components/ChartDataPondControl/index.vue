...
<script setup lang="ts">
  ...
  const createPond = () => {
    ...
    editData.value = {
      dataPondId: id,
      dataPondName: id,
      dataPondRequestConfig: cloneDeep({
        ...requestConfig,
  ...
  const editSaveHandle = (newData: RequestDataPondItemType) => {
    try {
      const targetIndex = requestDataPond.value.findIndex(
        (item) => item.dataPondId === newData.dataPondId
      );
      if (targetIndex !== -1) {
  ...
  const deletePond = (targetData: RequestDataPondItemType) => {
    goDialog({
      ...
      onPositiveCallback: () => {
        const targetIndex = requestDataPond.value.findIndex(
          (item) => item.dataPondId === targetData.dataPondId
        );
        if (targetIndex !== -1) {
