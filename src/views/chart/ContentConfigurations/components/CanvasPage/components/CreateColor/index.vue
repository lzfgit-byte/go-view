          ...
          <div class="color-list">
            ...
            <div class="color-card-box" v-for="(item, index) in colorList" :key="index">
              ...
              >
                ...
                <div
                  class="theme-bottom"
                  :style="{ backgroundImage: colorBackgroundImage(item) }"
                ></div>
              </n-card>
              <n-tooltip trigger="hover">
                <template #trigger>
                  <n-button
                    text
                    :disabled="item.id === selectThemeColor"
  ...
  const deleteHandle = (index: number) => {
    const positiveHandle = () => {
      ...
      chartEditStore.setEditCanvasConfig(
        EditCanvasConfigEnum.CHART_CUSTOM_THEME_COLOR_INFO,
        cloneDeep(colorList)
      );
      nextTick(() => {
        if (colorList.length) {
  ...
  const saveHandle = (onMessage = true) => {
    ...
    if (index !== -1) {
      ...
      const updateColorPrefix = cloneDeep({
        ...updateColor.value,
        name: updateColor.value.name || '未定义',
      });
