<template>
  <div class="create-color-setting" v-if="editColor">
            ...
            <div class="go-flex-items-center" :class="{ select: index === targetColor.index }">
              ...
              <div v-show="index > 5">
                <n-tooltip trigger="hover">
                  <template #trigger>
                    <n-icon
                      class="go-ml-1 go-cursor-pointer"
                      size="16"
                      :depth="3"
    ...
    <!-- 展示图表 -->
    <create-color-render-chart
      :color="cloneDeep(editColor.color).splice(0, 7)"
    ></create-color-render-chart>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const CreateColorRenderChart = loadAsyncComponent(
    () => import('../CreateColorRenderChart/index.vue')
  );
