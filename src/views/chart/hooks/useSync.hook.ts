...
// 接口
import { fetchProjectApi, saveProjectApi, updateProjectApi, uploadFile } from '@/api/path';
    ...
    for (const key in projectData) {
      ...
      if (key === ChartEditStoreEnum.COMPONENT_LIST) {
        ...
        for (const comItem of projectData[key]) {
          // 设置加载数量
          let percentage = Number.parseInt(
            (Number.parseFloat(`${++loadIndex / listLength}`) * 100).toString()
          );
