...
export const useChartInteract = (
  ...
  fnOnEvent.forEach((item) => {
    const globalConfigPindAprndex = chartEditStore.requestGlobalConfig.requestDataPond.findIndex(
      (cItem) => cItem.dataPondId === item.interactComponentId
    );
    if (globalConfigPindAprndex !== -1) {
      const { Params, Header } = toRefs(
        chartEditStore.requestGlobalConfig.requestDataPond[globalConfigPindAprndex]
