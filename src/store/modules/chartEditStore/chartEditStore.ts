...
// 历史类型
import type { HistoryItemType } from '@/store/modules/chartHistoryStore/chartHistoryStore.d';
import {
...
import { getUUID, isArray, isString, loadingError, loadingFinish, loadingStart } from '@/utils';
...
export const useChartEditStore = defineStore({
  ...
  actions: {
    // fix:[add]添加缺失的方法
    setProjectInfo<T extends keyof ProjectInfoType, K extends ProjectInfoType[T]>(
      key: T,
      value: K
    ) {
    ...
    setEditCanvasConfig<T extends keyof EditCanvasConfigType, K extends EditCanvasConfigType[T]>(
      key: T,
      value: K
    ) {
